...
By default, Gotenberg handles requests synchronously: you send a file, wait, and get a PDF back.
However, for large files or batch operations, you may prefer an **asynchronous** workflow.
...
You can customize retries, timeouts, and allowed URLs.
See the [Webhook module configuration](/docs/configuration#webhook) for details.

:::

:::tip Testing Webhooks

Tools like [PipeDream](https://pipedream.com/) or [Webhook.site](https://webhook.site) are excellent for testing these callbacks without setting up a dedicated server.
...
### Watermark & Stamp from Remote URLs

Use the `field` property to route a downloaded file as a watermark or stamp source:
  ...
  responses={[
    {
      status: 400,
      message: "Bad Request",
      description: "The JSON is malformed.",
      body: `
Content-Type: text/plain; charset=UTF-8
Gotenberg-Trace: {trace}
Body: {error}
