...
As you navigate through the documentation, feel free to open a terminal or [Bruno](https://www.usebruno.com/) and try
...
The Docker image runs as a non-root user named _gotenberg_, with a user ID (UID) and group ID (GID) of **1001**.
...
The `buffered` invoke mode supports responses of up to 6MB only.

For larger outputs, consider using the webhook feature to upload results to S3.

:::

:::info

There is an [ongoing discussion](https://github.com/gotenberg/gotenberg/issues/540) about improving this variant.
...
read the [routes guide](/docs/convert-with-chromium/convert-url-to-pdf) to learn more.
