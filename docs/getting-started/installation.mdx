...
To restrict access to the host only (for example when using gotenberg internally or behind a reverse proxy), binding to the loopback address might be a better option:
