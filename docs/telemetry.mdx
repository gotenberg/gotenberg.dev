...
Asynchronous webhook requests stay in the caller's trace. Since _8.33.0_, the background work runs under a `webhook.Async` span that links back to the originating request span, and the webhook delivery spans nest under it instead of starting a new trace. The span covers the whole background delivery, retries included.

Since _8.33.0_, traces carry richer detail:
...
Head sampling drops whole traces up front, including the rare slow or failed conversions that matter most for diagnosis. For high-throughput deployments, keep head sampling permissive and apply tail sampling in the collector, sampling on error or high latency. Since _8.33.0_, Gotenberg attaches trace-based exemplars to histograms, so conversion metrics still link to representative traces regardless of the sampling ratio.
