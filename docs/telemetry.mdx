...
Head sampling drops whole traces up front, including the rare slow or failed conversions that matter most for diagnosis. For high-throughput deployments, keep head sampling permissive and apply tail sampling in the collector, sampling on error or high latency. Since _8.33.0_, Gotenberg attaches trace-based exemplars to histograms: as long as some traces are sampled, conversion metrics link to a representative trace.
