...
Cookies expire when the request reaches its time limit. For strict isolation between conversions, configure the API to clear the cookie jar after every request. See [API module configuration](/docs/configuration#api).
...
A request carries at most 64 headers, and a `scope` pattern at most 1024 characters. Gotenberg matches every scoped pattern against every browser request, so it also caps the total time a conversion spends matching. Past that cap the remaining scoped headers are not sent. Keep the patterns simple: nested quantifiers such as `(a+)+` backtrack badly and exhaust the cap on their own.
