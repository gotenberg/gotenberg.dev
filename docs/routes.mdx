...
Most routes within Gotenberg are designed to accept _multipart/form-data_ requests and generate one or more PDF files.
...
This _multipart/form-data_ route converts a web page into PDF.
...
Gotenberg also returns a _400 Bad Request_ if it encounters any of the following network errors while attempting to load
the main page:
...
If a request doesn't complete within a specified duration, the API will respond with a _503 Service Unavailable_ status.
...
This _multipart/form-data_ route converts an HTML file into PDF.
...
This _multipart/form-data_ route converts Markdown file(s) into PDF.
...
If the `singlePage` form field is set to _true_, it automatically overrides the values from the `paperHeight` and
...
- If `printBackground` is set to _false_, no background is printed.
- If `printBackground` is set to _true_:
  - If the HTML document has a background, that background is used.
  - If not:
    - If `omitBackground` is set to _true_, the default background is transparent.
...
It is recommended to use a _large_ `font-size` to ensure that the content displays correctly.
...
| waitForExpression | The JavaScript expression to wait before converting an HTML document into PDF until it returns _true_. | None    |
...
You can add an optional _scope_ token to a header value to restrict its application using a regular expression.
...
Note that the _scope_ token is only processed by Gotenberg and is never sent with the header value.
...
| Key                             | Description                                                                                            | Default     |
| ------------------------------- | ------------------------------------------------------------------------------------------------------ | ----------- |
| failOnHttpStatusCodes           | Return a _409 Conflict_ response if the HTTP status code from the main page is not acceptable.         | \[499,599\] |
| failOnResourceHttpStatusCodes   | Return a _409 Conflict_ response if the HTTP status code from at least one resource is not acceptable. | None        |
| ignoreResourceHttpStatusDomains | Exclude resources from `failOnResourceHttpStatusCodes` checks based on their hostname.                 | None        |

An _X99_ entry means every HTTP status codes between _X00_ and _X99_ (e.g., 499 means every HTTP status codes between 400 and 499).
...
Gotenberg returns a _400 Bad Request_ if it encounters any of the following network errors while attempting to load
the main page:
...
| Key                         | Description                                                                       | Default |
| --------------------------- | --------------------------------------------------------------------------------- | ------- |
| failOnResourceLoadingFailed | Return a _409 Conflict_ response if Chromium fails to load at least one resource. | false   |
...
| Key                     | Description                                                                       | Default |
| ----------------------- | --------------------------------------------------------------------------------- | ------- |
| failOnConsoleExceptions | Return a _409 Conflict_ response if there are exceptions in the Chromium console. | false   |
...
Prior to Gotenberg _8.11.0_, `false` was the default value.
...
| Key        | Description                                                                                                                        | Default |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------- |
| splitMode  | Either _intervals_ or _pages_.                                                                                                     | None    |
| splitSpan  | Either the intervals or the page ranges to extract, depending on the selected mode.                                                | None    |
| splitUnify | Specify whether to put extracted pages into a single file or as many files as there are page ranges. Only works with _pages_ mode. | false   |

:::caution

Gotenberg currently does not validate the `splitSpan` value if `splitMode` is _pages_, as it depends on the selected
...
As of Gotenberg _8.21.0_, using the `generateTaggedPdf` form field may yield better results if you're prioritizing
...
### Encrypt {#encrypt-chromium}

Each route accepts the following specific form fields:
...
### Embed Files {#embeds-chromium}

This feature enables the creation of PDFs compatible with standards like [ZUGFeRD / Factur-X](https://fnfe-mpe.org/factur-x/), which require embedding XML invoices and other files within the PDF.
...
If your screenshots' content is repeated and clipped, consider setting the `skipNetworkIdleEvent` form field to _false_.
...
This _multipart/form-data_ route convert one or more Office documents into PDF.
...
| exportOnlyNotesPages            | Specify, if the form field _exportNotesPages_ is set to true, if only notes pages are exported to PDF.                                                                        | false     |
...
| reduceImageResolution    | Specify if the resolution of each image is reduced to the resolution specified by the form field _maxImageResolution_.                                                   | false   |
| maxImageResolution       | If the form field _reduceImageResolution_ is set to true, tell if all images will be reduced to the given value in DPI. Possible values are: 75, 150, 300, 600 and 1200. | 300     |
...
### Encrypt {#encrypt-libreoffice}

The route accepts the following specific form fields:
...
### Embed Files {#embeds-libreoffice}
...
This _multipart/form-data_ route transforms one or more PDF files into the requested PDF/A format and/or PDF/UA.
...
<span style={{ color: "red" }}>\*</span>Note that at least one of the form field
must be provided.
...
This _multipart/form-data_ route returns the metadata of one or more PDF files.
...
This _multipart/form-data_ route accepts one or more PDF files and writes/overrides their metadata.
...
This _multipart/form-data_ route accepts PDF files and merges them alphanumerically.
...
This _multipart/form-data_ route splits PDF files.
...
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| \*.pdf        | At least one PDF file.                                                                                                             | <span class="badge badge--danger">required</span> |
| splitMode     | Either _intervals_ or _pages_.                                                                                                     | <span class="badge badge--danger">required</span> |
...
This _multipart/form-data_ route flatten PDF files.
...
## Encrypt <span class="badge badge--secondary">route</span>

This _multipart/form-data_ route accepts one or more PDF files and adds password protection to them.
...
| Key           | Description                                       |                                                   |
| ------------- | ------------------------------------------------- | ------------------------------------------------- |
| files         | At least one PDF file.                            | <span class="badge badge--danger">required</span> |
| userPassword  | Password for opening the resulting PDF(s).        | <span class="badge badge--danger">required</span> |
| ownerPassword | Password for full access on the resulting PDF(s). | None                                              |
...
## Embed Files <span class="badge badge--secondary">route</span>

This feature enables the creation of PDFs compatible with standards like [ZUGFeRD / Factur-X](https://fnfe-mpe.org/factur-x/), which require embedding XML invoices and other files within the PDF.

This _multipart/form-data_ route accepts one or more PDF files and embeds files into them.
...
| Key    | Description                             |                                                   |
| ------ | --------------------------------------- | ------------------------------------------------- |
| files  | PDF files to embed files into.          | <span class="badge badge--danger">required</span> |
| embeds | Files to embed in the PDF (repeatable). | <span class="badge badge--danger">required</span> |
...
This route is only available if the flag `--api-enable-debug-route` is set to _true_.
...
By default, for _multipart/form-data_ endpoints, the API generates a response with a UUID filename.
...
All _multipart/form-data_ endpoints accept the following form field:
...
| Key              | Description                                                                                     | Default                                           |
| ---------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| url              | URL of the file. It **MUST** return a `Content-Disposition` header with a _filename_ parameter. | <span class="badge badge--danger">required</span> |
| extraHttpHeaders | The extra HTTP headers to send to the URL (JSON format).                                        | None                                              |
| embedded         | Whether this file should be embedded in the PDF (see [Embed Files](#embed-files-route)).        | false                                             |
...
If you're using AWS S3, make sure to set the `Content-Disposition` header with the _attachment_ value in the S3 metadata.
