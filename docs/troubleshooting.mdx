...
- On macOS, consider disabling the _Use Virtualization Framework_ ([Issue #792](https://github.com/gotenberg/gotenberg/issues/792)).
...
- For versions after _8.11.0_, set the `skipNetworkIdleEvent` field to _false_.
...
- Set the `skipNetworkIdleEvent` field to _false_ to resolve this issue
...
- Starting with version 7.6, LibreOffice no longer supports _PDF/A-1a_. It previously generated _PDF/A-1b_ files, though
