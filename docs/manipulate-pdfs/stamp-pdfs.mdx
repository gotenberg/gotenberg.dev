...
Adds a stamp _on top of_ the content of each page of one or more PDF files.

A stamp is rendered above the existing page content, overlaying it.
Use `text` for a simple label, `image` to overlay an uploaded image, or `pdf` to stamp a full-page PDF on top.
