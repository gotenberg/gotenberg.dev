...
Since _8.33.0_, LibreOffice drops linked content: an uploaded document always loads from an untrusted location, so linked `file://` paths and external URLs are not resolved. The conversion still returns `200 OK` with the linked content absent. There is no opt-out; embedded content is unaffected, so embed images instead of linking them.
