...
### Metadata, Flatten & Attachments (PDF Engines)
...
Set `exportFormFields` to false to prevent LibreOffice from creating interactive PDF widgets. Happens _during_ conversion — faster than post-processing.
...
Set `flatten` to true to make the PDF non-interactive _after_ generation. Useful when combining flattening with other post-processing operations like merging or metadata extraction.
