...
Gotenberg auto-detects some files you may send in your request: `.woff2`, `.woff`, `.ttf`, `.css` and `.js`.

It adds on the fly the corresponding `<link>` and `<script>` HTML elements inside the DOM of the target URL.
...
Gotenberg adds these HTML elements alphabetically, according to the filenames.

:::

:::caution

`<script>` elements are not added if JavaScript is disabled via `--chromium-disable-javascript`.

:::

---

The form fields `extraLinkTags` and `extraScriptTags` (JSON format) allows you to add remote paths and override the
...
For `<link>` elements, only the `href` attribute is available.

:::

:::info

For `<script>` elements, only the `src` attribute is available.

:::

:::info

Gotenberg adds the remaining files that it auto-detects after the assets from `extraLinkTags` and `extraScriptTags`.
