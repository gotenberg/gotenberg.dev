...
--uno-listener-restart-threshold int       Operations limit after which the LibreOffice listener is restarted - 0 means no long-running LibreOffice listener (default 10)
```
...
The `--uno-*` properties also impact the performance of the [LibreOffice](libreoffice) module.
