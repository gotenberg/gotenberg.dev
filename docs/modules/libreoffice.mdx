...
The LibreOffice listener is also restarted if it is unhealthy before and after a conversion.
...
If there is a long-running LibreOffice listener and it is unhealthy, the [health route](api#health) returns a
`503 Service Unvailable` response.

:::info

A restarting LibreOffice listener is considered healthy.
