...
- LibreOffice fetches for document references (OOXML external images, RTF `INCLUDEPICTURE`, ODT linked images). Since _8.33.0_, content linked from uploaded documents is dropped at the source; the filters below cover any fetch LibreOffice still performs.
...
**Linked content dropped in LibreOffice documents.** Since _8.33.0_, LibreOffice refuses to load content linked from an untrusted location. Uploaded documents always load from a per-request temporary directory, which is never trusted, so linked `file://` paths and external URLs are dropped before any fetch happens. An allow-list match cannot re-enable them. Embedded content stored inside the document is unaffected.
